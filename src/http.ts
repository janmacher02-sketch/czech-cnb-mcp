...
app.get("/", (_req, res) => {
  res.json({ name: "czech-cnb-mcp", version: "1.0.0", status: "ok" });
});

app.post("/mcp", authMiddleware, async (req, res) => {
  const server = new McpServer({ name: "czech-cnb-mcp", version: "1.0.0" });
...
app.listen(PORT, () => console.log(`Czech CNB MCP running on port ${PORT}`));
